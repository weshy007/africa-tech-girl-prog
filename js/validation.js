    ...
    window.addEventListener('load', function() { //level 1 start
      var form = document.getElementById('frmContact'); //receive the Contact form with id frmContact
          ...
          if (form.checkValidity() === false) { //execute lines of code within the if, if the form fails validation
            ...
            var upperUsername = userName.charAt(0).toUpperCase() + userName.slice(1) // capitalise first letter of a name

            var userEmail = document.getElementById("userEmail").value;
            var subject = document.getElementById("subject").value;
            var content = document.getElementById("message").value;
            alert("Here is what we got from you, "+ upperUsername + ".\nSubject: "+ subject + "\nEmail: " + userEmail + "\nMessage: "+content);
            alert("Thank you "+upperUsername+". Your message has been sent."); //Give personalized Feedback
          }
        }, false); //level 2 end
    }, false); //level 1 end
  })(); //end of function 
